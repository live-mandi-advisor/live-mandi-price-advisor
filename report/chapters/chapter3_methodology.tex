...
on reporting consistency (days/markets reported), not raw row count
alone, so that a commodity with sporadic gaps is not favoured over one
with steady daily coverage.
...
\bottomrule
\caption{Finalized state and top-6-crop selection, derived from
reporting-consistency ranking on the 2024+2025 backfill data.}
